...
async function initialiseRoleCache(bgCore: BugtrackCore) {
	// Fetch all roles from the database.
	const roleIDs: QueryResult<{ roleid: string }> = await gpPool.query(
		'SELECT roleid FROM roles WHERE deleted = $1;',
		[false],
	);
